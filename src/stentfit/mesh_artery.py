"""Step 4-5: artery solid meshing (GMSH) + beam-to-solid assembly (BeamMe).

Turns the *surface* artery (from ``generate_artery``) into a 3D **solid tet
mesh** and combines it with the warped 1D stent beam mesh into a single BeamMe
``Mesh`` with a beam-to-solid coupling condition, ready to write as a 4C input.

Three pieces:

* ``mesh_artery_cubit`` was the original route, but Coreform Cubit dropped macOS
  support (Jan 2025), so we mesh with **GMSH** instead (open-source, pip-installable,
  Mac-native). ``mesh_artery_gmsh`` builds the hollow artery wall as ``TET4``,
  ``TET10`` or structured ``HEX8`` and writes a **4C ``.yaml``** solid (nodes +
  ``SOLID <TYPE>`` elements + ``DSURF`` sets for the lumen / inlet / outlet).
* ``import_artery_solid`` — reads that 4C file (or a CubitPy model) into a BeamMe
  ``Mesh`` with the volume elements + surface geometry sets.
* ``assemble_beam_solid`` — adds the warped stent beams, ties them to the lumen
  surface with a beam-to-solid condition, and dumps the assembled ``.4C.yaml``.

Heavy deps (``gmsh``, ``beamme``, ``fourcipp``, ``cubitpy``) are imported lazily
inside the functions so the rest of ``stentfit`` imports without them, mirroring
``mesh_skeleton_beams`` in ``stent_splines``.
"""
...
def _warp_coords_to_centreline(coords, centreline):
    ...
    """
    from .process_funcs import _compute_rmf
