...
def preprocess_stent(
    ...
    if remove_supports:
        ...
        ## NOTE: Keep from points where |z| < 4.5
        pts     = pts[np.abs(z_cyl) < 4.5]
        shifted = shifted[np.abs(z_cyl) < 4.5]
        r       = r[np.abs(z_cyl) < 4.5]
        theta   = theta[np.abs(z_cyl) < 4.5]
        z_cyl   = z_cyl[np.abs(z_cyl) < 4.5] 
...
def find_crowns(
    ...
    if not segmentation_wanted:
        stent_df['crown_id'] = 1
        crown_allowed     = np.array([[True]])
        whole_stent_crown = np.array([[True]])
        print("[crown] splitting skipped — all points in one crown")
        return {
            'stent_df'          : stent_df,
            'crown_allowed'     : crown_allowed,
            'whole_stent_crown' : whole_stent_crown,
            'crown_edges'       : np.array([stent_df['z_cylindrical'].min(),
                                            stent_df['z_cylindrical'].max()]),
            'n_crowns'          : 1,
            'conn_radius_3d'    : 0.0,
        }

    whole_stent_crown = np.array([[True]])  # all points belong to the same whole stent
